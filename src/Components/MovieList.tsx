...
export function MovieList({ number, input }: INumber) {
  const [location, setLocationValue] = useState(0);
  useEffect(() => {
    setLocationValue(number);
  }, []);
  ...
  return (
    <div>
      <Slider style={{ top: location }} className="first">
        <AnimatePresence
          custom={back}
          initial={false}
